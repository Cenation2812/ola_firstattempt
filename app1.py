...
def driverdetails():
    ...
    dit["Start location"] = {"dslat":data.get("dslat"),"dslong":data.get("dslong")}
    dit["Max distance"] = {"lat":data.get("drlat"),"long":data.get("drlong")}
...
def searchdriver():
    ...
    for doc in docs:
        if doc.to_dict().get("name") == Passname:
    ...
    for doc in docs:
        ...
        if pickupdistance <= driverdistance:
            ...
            if traveldistance <= driverdistance:
                ...
                return jsonify("No driver found for taking you to your destination")
